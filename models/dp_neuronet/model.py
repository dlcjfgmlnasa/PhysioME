    ...
    def __init__(self, fs: int, second: int, time_window: int, time_step: float,
        ...
        for i, (h1, h2) in enumerate(zip(projection_hidden[:-1], projection_hidden[1:])):
            if i != len(projection_hidden) - 2:
                ...
                projectors.append(nn.Linear(h1, h2))
        self.projectors1 = nn.Sequential(*projectors)
        self.projectors2 = nn.Sequential(*projectors)
        self.projectors_bn = nn.BatchNorm1d(projection_hidden[-1], affine=False)
        self.norm_pix_loss = False

    def forward(self, x1: torch.Tensor, x2: torch.Tensor, mask_ratio: float = 0.5):
        recon_loss1, contrastive_loss1 = self.single_forward(x1, mask_ratio=mask_ratio)  # NeuroNet 1
        recon_loss2, contrastive_loss2 = self.single_forward(x2, mask_ratio=mask_ratio)  # NeuroNet 2
        recon_loss = (recon_loss1 + recon_loss2) / 2
        contrastive_loss1 = (contrastive_loss1 + contrastive_loss2) / 2

        x1, x2 = self.make_frame(x1), self.make_frame(x2)
        x1, x2 = self.frame_backbone(x1), self.frame_backbone(x2)
        latent1 = self.autoencoder.forward_encoder(x1, mask_ratio=mask_ratio)[0]
        latent2 = self.autoencoder.forward_encoder(x2, mask_ratio=mask_ratio)[0]
        o1, o2 = latent1[:, :1, :].squeeze(), latent2[:, :1, :].squeeze()
        z1, z2 = self.projectors2(o1), self.projectors2(o2)
        contrastive_loss2 = self.contrastive_loss(z1, z2)
        return recon_loss, contrastive_loss1, contrastive_loss2

    def single_forward(self, x: torch.Tensor, mask_ratio: float = 0.5) -> (torch.Tensor, torch.Tensor):
        x = self.make_frame(x)
        ...
        z1, z2 = self.projectors1(o1), self.projectors1(o2)
...
class NeuroNetEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # add pos embed w/o cls token
        pos_embed_interp = interpolate_1d_pos_embed(self.pos_embed, target_len=l, cls_token=True)

        x = x + pos_embed_interp[:, 1:, :]
...
if __name__ == '__main__':
    m0 = NeuroNet(fs=100, second=60, time_window=3, time_step=3,
                  encoder_embed_dim=256, encoder_depths=8, encoder_heads=8,
                  decoder_embed_dim=128, decoder_depths=4, decoder_heads=4,
                  projection_hidden=[1024, 512])
    oo = m0.forward(torch.randn((50, 6000)),
                    torch.randn((50, 6000)),
                    mask_ratio=0.75)
    print(oo)
